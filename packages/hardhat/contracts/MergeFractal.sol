...
contract MergeFractal is ERC721, Ownable {
  ...
  // 11 bits of offset
  function renderLines(uint256 id, uint8 colourSection, uint8 startBit) public view returns (string memory) {
    ...
    // string memory col0 = string(abi.encodePacked(getRGBA(id, startBit, colourSection, "0.99"), strokeAndLine));

    // string memory col0 = string(abi.encodePacked(getRGBA(id, startBit, colourSection, "0.99"), '" stroke-width="1px" d="',linesPath,'"/>'));
