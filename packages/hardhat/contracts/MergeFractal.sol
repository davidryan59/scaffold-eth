...
contract MergeFractal is ERC721, Ownable {
  ...
  mapping (uint256 => bytes32) public generator;
      ...
      returns (uint256)
  {
      ...
      generator[id] = keccak256(abi.encodePacked( blockhash(block.number-1), msg.sender, address(this), id ));

      // bytes32 predictableRandom = keccak256(abi.encodePacked( blockhash(block.number-1), msg.sender, address(this), id ));
      // generator[id] = predictableRandom;

      color1[id] = bytes2(generator[id][0]) | ( bytes2(generator[id][1]) >> 8 ) | ( bytes3(generator[id][2]) >> 16 );
      color2[id] = bytes2(generator[id][3]) | ( bytes2(generator[id][4]) >> 8 ) | ( bytes3(generator[id][5]) >> 16 );
      color3[id] = bytes2(generator[id][6]) | ( bytes2(generator[id][7]) >> 8 ) | ( bytes3(generator[id][8]) >> 16 );
  ...
  function tokenURI(uint256 id) public view override returns (string memory) {
      ...
      string memory description = string(abi.encodePacked('This Merge Fractal is the color #',color1[id].toColor(),'!!!'));
  ...
  function renderTokenById(uint256 id) public view returns (string memory) {
    ...
    string memory render = string(abi.encodePacked(
      ...
      // GOLDS
      '<path id="lines1" fill="none" stroke="',ToColor.toRGBA(color1[id],"0.55"),'" stroke-width="1.5px" d="',linesPath,'"/>',
      '<path id="lines3" fill="none" stroke="',ToColor.toRGBA(color2[id],"0.55"),'" stroke-width="1.5px" d="',linesPath,'"/>',
      '<path id="lines2" fill="none" stroke="',ToColor.toRGBA(color3[id],"0.55"),'" stroke-width="1.5px" d="',linesPath,'"/>',
      ...
      '<circle fill="rgba(255, 255, 255, 1)" cx="200" cy="200" r="200"/>',
      ...
      // this looks great
      '<g><animateTransform attributeName="transform" attributeType="XML" type="rotate" values="0 200 200; -180 200 200; 0 200 200" dur="  7s" repeatCount="indefinite"/><use href="#lines2"/></g>',
      '<g><animateTransform attributeName="transform" attributeType="XML" type="rotate" values="0 200 200;  180 200 200; 0 200 200" dur=" 11s" repeatCount="indefinite"/><use href="#lines1"/></g>',
